...
struct Flip<Front, Back>: View where Front: View, Back: View {
    ...
    // isFlipped with in underscore to indicate that we're assigning isFlipped to the wrapper (the Binding struct itself)
    // which has a type of Binding<Bool>, not to the instance, which just has a type of Bool.
